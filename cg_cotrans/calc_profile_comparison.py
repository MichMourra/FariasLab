...
if __name__ == '__main__':
    ...
    excluded_genes = set()
    if clargs.blacklist != None:
        with open(clargs.blacklist, 'r') as f:
    ...
    for gene in clargs.genes:
        ...
        if gene in excluded_genes:
            ...
            continue
        try:
            rc_data = read_rc_profile('%s_rc_profile.dat' % gene)
        except IOError:
            rc_data = None
            print("WARNING: could not find rare-codon profile data for gene %s" % gene)
            continue
        try:
            offset, nvertices, fe = read_elongation_profile('%s_elongation_profile.dat' % gene)
            if len(fe) > rc_data['N']:
                print("WARNING: skipping gene %s with inconsistent profile lengths" % gene)
                continue
        except IOError:
            continue
        if rc_data['N'] > clargs.Lmax:
            print("WARNING: skipping gene %s with L > Lmax = %d" % (gene, clargs.Lmax))
            continue
        data[gene] = rc_data
    ...
    for dataset_name, data in datasets.items():
        print("Writing %sgenes_%s.lst" % (clargs.output_prefix, dataset_name))
        with open('%sgenes_%s.lst' % (clargs.output_prefix, dataset_name), 'w') as f:
            for gene in sorted(data):
                f.write("%s\n" % gene)

        output_keys = ['frac_explained']
        print("Writing %scompare_profiles_%s_explain.dat" % (clargs.output_prefix, dataset_name))
        with open('%scompare_profiles_%s_explain.dat' % \
                  (clargs.output_prefix, dataset_name), 'w') as f:
            f.write("# frac_seq_min pval_max n")
            for key in output_keys:
                f.write(" %s %s %s" % (key, key + '_err', \
                                       ' '.join("%s %s" % (key + '_control_%s_mean' % k, \
                                                           key + '_control_%s_std' % k) \
        ...
        print("Writing %scompare_profiles_%s_predict.dat" % (clargs.output_prefix, dataset_name))
